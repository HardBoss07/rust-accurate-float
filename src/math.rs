use crate::util::{decode_as_tuple, bit_length, f_max, w_max};
...
pub fn add_two(numb_a: u32, numb_b: u32) {
    ...
    let f_max_len = f_max(bit_length(decoded_a.2), bit_length(decoded_b.2));
    let w_max_len = w_max(bit_length(decoded_a.1), bit_length(decoded_b.1));
}
